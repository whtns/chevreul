...
#' )
getEnrichedPathways <- function(object, column_sample = "sample", column_cluster = "cluster",
                                 databases = c("GO_Biological_Process_2018", "GO_Cellular_Component_2018",
                                               "GO_Molecular_Function_2018", "KEGG_2016", "WikiPathways_2016",
                                               "Reactome_2016", "Panther_2016", "Human_Gene_Atlas",
                                 ...
                                 URL_API = "http://amp.pharm.mssm.edu/Enrichr/enrich")
{
  ...
  if (is.null(object@misc$marker_genes)) {
    stop("No marker genes found. Please run 'getMarkerGenes()' first.",
         call. = FALSE)
  }
  if (!is.null(object@misc$marker_genes$by_sample)) {
    if (is.data.frame(object@misc$marker_genes$by_sample)) {
      ...
      results_by_sample <- future.apply::future_sapply(sample_names,
                                                       USE.NAMES = TRUE, simplify = FALSE, future.globals = FALSE,
                                                       function(x) {
                                                         temp <- list()
                                                         attempt <- 1
                                                         while ("Adjusted.P.value" %in% names(temp) ==
                                                                FALSE && attempt <= 3) {
                                                           attempt <- attempt + 1
                                                           try(temp <- markers_by_sample %>% dplyr::filter(.data$sample ==
                                                                                                             x) %>% dplyr::select("gene") %>% t() %>%
                                                                 as.vector() %>% .send_enrichr_query(databases = databases,
                                                                                                     URL_API = URL_API))
                                                         }
                                                         results_2 <- sapply(names(temp), USE.NAMES = TRUE,
                                                                             simplify = FALSE, function(y) {
                                                                               out <- temp[[y]] %>% dplyr::filter(.data$Adjusted.P.value <=
                                                                                                                    adj_p_cutoff) %>% dplyr::mutate(db = y)
                                                                               if (nrow(out) > max_terms) {
                                                                                 out <- out %>% dplyr::top_n(-max_terms,
      ...
      results_by_sample <- do.call(rbind, results_by_sample) %>%
        ...
        dplyr::mutate(sample = factor(.data$sample,
                                      levels = intersect(sample_names, .data$sample)),
      ...
      warning(paste0("Unexpected data format of marker genes for samples. Please submit ",
  ...
  if (!is.null(object@misc$marker_genes$by_cluster)) {
    if (is.data.frame(object@misc$marker_genes$by_cluster)) {
      ...
      results_by_cluster <- future.apply::future_sapply(cluster_names,
                                                        USE.NAMES = TRUE, simplify = FALSE, future.globals = FALSE,
                                                        function(x) {
                                                          temp <- list()
                                                          attempt <- 1
                                                          while ("Adjusted.P.value" %in% names(temp) ==
                                                                 FALSE && attempt <= 3) {
                                                            attempt <- attempt + 1
                                                            try(temp <- markers_by_cluster %>% dplyr::filter(.data$cluster ==
                                                                                                               x) %>% dplyr::select("gene") %>% t() %>%
                                                                  as.vector() %>% .send_enrichr_query(databases = databases,
                                                                                                      URL_API = URL_API))
                                                          }
      ...
      for (i in names(results_by_cluster)) {
        if (is.null(results_by_cluster[[i]]))
          results_by_cluster[[i]] <- NULL
      }
      for (i in names(results_by_cluster)) {
      ...
      results_by_cluster <- do.call(rbind, results_by_cluster) %>%
        ...
        dplyr::mutate(cluster = factor(.data$cluster,
                                       levels = intersect(cluster_names, .data$cluster)),
                      db = factor(.data$db, databases))
      message(paste0("[", format(Sys.time(), "%H:%M:%S"),
                     "] ", nrow(results_by_cluster), " pathways passed the thresholds across all clusters and databases."))
    }
      ...
      warning(paste0("Unexpected data format of marker genes for clusters. Please submit ",
                     "an issue on GitHub: https://github.com/romanhaa/cerebroApp."))
    }
  }
  else {
  ...
  results <- list(by_sample = results_by_sample, by_cluster = results_by_cluster,
                  parameters = list(databases = databases, adj_p_cutoff = adj_p_cutoff,
                                    max_terms = max_terms))
