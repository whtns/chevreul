...
scater::plotPCA(myobject0, color_by = "nFeature_gene")
...
Chevreul also includes a separate function `object_reduce_dimentions()` capable of implementing a standardized dimension reduction step to select variable features at a user-specified threshold. This function performs tSNE and UMAP in addition to PCA. The default assay the dimension reduction is being run on is "gene".

```{r, results=FALSE}
myobject_red <- object_reduce_dimensions(myobject0)
scater::plotUMAP(myobject_red, color_by = "nFeature_gene")
```
...
A separate Chevreul function, `object_cluster()`, can be used to perform clustering of scRNA-seq data. Here, clustering analysis is performed via Louvain algorithms, using exsisting functions that perform graph-based clustering. Clustering is performed at a range of resolutions with default value ranging from 0.2 to 2 and PCA reduction.

```{r, results=FALSE, eval=TRUE}
object <- object_cluster(object = myobject_red, 
                         resolution = seq(0.2, 2, by = 0.2), reduction = "PCA")
scater::plotPCA(object, color_by = "nFeature_gene")
```
