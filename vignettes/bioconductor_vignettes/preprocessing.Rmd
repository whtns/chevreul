...
Chevreul also includes a function `object_reduce_dimentions()` which implements a standardized dimension reduction step to select variable features at a user-specified threshold and perform PCA, tSNE, and UMAP. The default assay the dimension reduction is being run on is "gene".

```{r, results=FALSE}
myobject <- object_reduce_dimensions(myobject)
DimPlot(myobject)
```
...
```{r, results=FALSE}
Dim_Red_object <- runPCA(x = myobject, 
...
Clustering of scRNA-seq data is performed by Chevreul using the `object_cluster()` function. Clustering analysis is performed via Louvain algorithms, using exsiting functions that perform graph-based clustering. Clustering is performed at a range of resolutions with default value ranging from 0.2 to 2 and pca reduction
