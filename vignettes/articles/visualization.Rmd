...
title: "Visualization"
---
This article demonstrates data visualization tools present in Chevreul. Through this article we will be introducing visualizations functions, how they work and the resulting out of these functions. 
...
The plot_readcount function in Cheverul plots a bar plot of number of read counts present in a metadata variable for each sample in the Seurat object. Here, the bars of the plot can be colored based on categorical metadata variable using the argument color.by whose default value is set to "batch"
...
```{r}
plot_var(clustered_seu, return_plotly = FALSE)
