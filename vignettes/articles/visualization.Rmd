...
- When plotting only one feature, an existing Seurat function is used. This function colors single cells on a dimensional reduction plot according to a 'feature'.

```{r}
FeaturePlot(clustered_seu, features = "RXRG")

```

- If multiple features are provided, the joint density of all the features are plotted using a function from the Nebulosa package.

```{r}
Nebulosa::plot_density(clustered_seu, features = c("RXRG", "NRL"), combine = "FALSE")

```

- Once a feature plot of produced, this plot is then converted to an interactive plot using plotly
